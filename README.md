...
* 📄 **MIME 類型查詢對照表 (`mime-type`)**：
  * 完整收錄 **80+ 種 MIME 類型**，涵蓋文本、圖片、影音、字型、壓縮檔、Office 文件、OpenDocument 等全類別。
  * 包含 HTTP Header 範例、詳細說明與伺服器配置指引（Nginx / Express / HTML）。
...
* 🔄 **文字重複移除工具 (`text-dedup`)**：
  * 支援按行、按單字、按字元三種去重模式，可忽略大小寫、移除空白行與排序。
...
* 📅 **日期計算器 (`date-calculator`)**：
  * 計算兩日期差距（天數、小時、週、月、年），支援工作天計算（排除週末）。
  * 從指定日期加減天數推算目標日期，顯示中文星期與完整格式。
* 💱 **貨幣轉換器 (`currency-converter`)**：
  * 即時匯率換算，支援全球 30+ 種主要貨幣，資料來源為公開免費 API。
  * 快速幣別切換、對調功能與熱門幣別匯率一覽表。
